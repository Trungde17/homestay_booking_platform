...
public class FeedbackDAO extends DAO {
    ...
    private static final String INSERT_FEEDBACK_SQL = "INSERT INTO tblFeedback (ht_id, customer_id, customerName, comments, rating, createdDate) VALUES (?, ?, ?, ?, ?, ?);";
    private static final String SELECT_FEEDBACK_BY_ID = "SELECT id, ht_id, customer_id, customerName, comments, rating, createdDate FROM tblFeedback WHERE id = ?";
    ...
    private static final String UPDATE_FEEDBACK_SQL = "UPDATE tblFeedback SET ht_id = ?, customerName = ?, comments = ?, rating = ?, createdDate = ? WHERE id = ?;";
    ...
    public static boolean insertFeedback(Feedback feedback) throws SQLException {
        ...
        try ( Connection connection = getConnection();  PreparedStatement preparedStatement = connection.prepareStatement(INSERT_FEEDBACK_SQL)) {
            preparedStatement.setInt(1, feedback.getHt_id());
            preparedStatement.setInt(2, feedback.getCustomer_id());
            preparedStatement.setString(3, feedback.getCustomerName());
            preparedStatement.setString(4, feedback.getComments());
            preparedStatement.setInt(5, feedback.getRating());
            preparedStatement.setDate(6, Date.valueOf(feedback.getCreatedDate()));
    ...
    public static Feedback selectFeedback(int id) throws SQLException {
        ...
        try ( Connection connection = getConnection();  PreparedStatement preparedStatement = connection.prepareStatement(SELECT_FEEDBACK_BY_ID);) {
            ...
            while (rs.next()) {
                ...
                feedback = new Feedback(id, ht_id, customer_id, customerName, comments, rating, createdDate);
            }
        } catch (SQLException e) {
            printSQLException(e);
        }
    ...
    public static List<Feedback> selectAllFeedback() {
        ...
        try ( Connection connection = getConnection();  PreparedStatement preparedStatement = connection.prepareStatement(SELECT_ALL_FEEDBACK);) {
            ...
            while (rs.next()) {
                ...
                int rating = rs.getInt("rating");
                LocalDate createdDate = rs.getDate("createdDate").toLocalDate();
                feedbackList.add(new Feedback(id, ht_id, customer_id, customerName, comments, rating, createdDate));
            }
        } catch (SQLException e) {
            printSQLException(e);
        }
    ...
    public static boolean updateFeedback(Feedback feedback) throws SQLException {
        ...
        try ( Connection connection = getConnection();  PreparedStatement statement = connection.prepareStatement(UPDATE_FEEDBACK_SQL);) {
            statement.setInt(1, feedback.getHt_id());
